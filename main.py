...
# ----------------------------------------------------------
#
# Power BI report authors can put dataset RLS on any column, not
# just a region - the caller tells us which column (rls_column) and
# which value(s) the current viewer is authorized to see
# (rls_values), and this builds the "<COLUMN> IN (...)" fragment
# used to steer Cortex Analyst / the RCA model, escaping the values
# and restricting the column name to safe identifier characters.
# ----------------------------------------------------------

_SAFE_IDENTIFIER_RE = re.compile(r"[^A-Za-z0-9_]")


def format_rls_filter(
    rls_column: Optional[str],
    rls_values: Optional[List[str]]
) -> Optional[str]:

    if not rls_column or not rls_values:
        return None

    safe_column = _SAFE_IDENTIFIER_RE.sub("", rls_column)
    ...
    return f"{safe_column} IN ({formatted_values})"
...
def get_active_semantic_view():
    ...
    #to use semantic view created by user put force_env_semantic_view = "false"
...
def generate_rca_ai_response(
    ...
    rls_context = format_rls_filter(rls_column, rls_values) or "Not provided"
...
The authorized row-level-security (RLS) filter is:
...
- The RLS filter above is an authorized security boundary.
- Do not suggest or infer information outside that authorized filter.
- Do not request or assume data outside the authorized RLS filter.
- The supplied SQL already contains the authorized RLS filtering.
...
"""
    ...
    try:
        ...
        print(
            f"RCA authorized RLS filter: {rls_context}"
        )
...
# ==========================================================
# ASK CORTEX EXTERNAL SHARE / EMBED
# ==========================================================
#
# The normal Snowflake Streamlit viewer URL requires the viewer
# to sign in to Snowflake.  For a public/no-Snowflake-login
# experience, the backend hosts a share page and mints a fresh
# one-time Snowflake Streamlit embed URL server-side.
#
# Required backend environment variables:
#
#   SNOWFLAKE_EMBED_PAT
#   SNOWFLAKE_EMBED_ROLE
#   ASK_CORTEX_PARENT_ORIGIN
#
# Optional:
#
#   SNOWFLAKE_EMBED_ACCOUNT_URL
#   ASK_CORTEX_DATABASE
#   ASK_CORTEX_SCHEMA
#   ASK_CORTEX_STREAMLIT
#
# The existing SNOWFLAKE_PAT / SNOWFLAKE_ROLE / SNOWFLAKE_ACCOUNT
# values are used only as fallbacks so the rest of the backend
# remains unchanged.  For production, use a dedicated SERVICE
# user PAT with USAGE + EMBED on IBP_ASK_CORTEX.
# ==========================================================

ASK_CORTEX_DATABASE = os.getenv(
    "ASK_CORTEX_DATABASE",
    "CPG"
)

ASK_CORTEX_SCHEMA = os.getenv(
    "ASK_CORTEX_SCHEMA",
    "IBP_SEMANTIC"
)

ASK_CORTEX_STREAMLIT = os.getenv(
    "ASK_CORTEX_STREAMLIT",
    "IBP_ASK_CORTEX"
)


def _get_snowflake_account_url_for_embed():
    """
    Return the Snowflake account URL used by the
    generate-embed-url REST API.
    """

    configured_url = (
        os.getenv("SNOWFLAKE_EMBED_ACCOUNT_URL")
    ...
    if configured_url:
        ...
        return f"https://{configured_url}"

    account = os.getenv("SNOWFLAKE_ACCOUNT", "").strip()

    if not account:
        raise RuntimeError(
            "SNOWFLAKE_ACCOUNT or SNOWFLAKE_EMBED_ACCOUNT_URL "
            "must be configured."
        )
...
def _get_ask_cortex_embed_credentials():
    """
    Return the dedicated credential/role used only for minting
    Ask Cortex external embed URLs.

    A dedicated service-user PAT is strongly recommended.
    """

    token = (
        os.getenv("SNOWFLAKE_EMBED_PAT")
        or os.getenv("SNOWFLAKE_PAT")
    )

    role = (
        os.getenv("SNOWFLAKE_EMBED_ROLE")
        or os.getenv("SNOWFLAKE_ROLE")
    )

    if not token:
        raise RuntimeError(
            "SNOWFLAKE_EMBED_PAT is not configured."
        )

    if not role:
        raise RuntimeError(
            "SNOWFLAKE_EMBED_ROLE is not configured."
        )

    return token.strip(), role.strip()


def _get_ask_cortex_parent_origin(request: Request):
    """
    Return the exact browser origin that will contain the iframe.

    In production this should be explicitly configured through
    ASK_CORTEX_PARENT_ORIGIN and registered in Snowflake's
    STREAMLIT_EMBEDDING_CONTROLS account parameter.

    The request origin is used only as a local-development fallback.
    """
    ...
    # Useful for local development.  In production, explicitly set
    # ASK_CORTEX_PARENT_ORIGIN so reverse proxies do not produce a
    # different internal origin.
    return str(
        request.base_url
    ).rstrip("/")


def _mint_ask_cortex_embed_url(request: Request):
    """
    Mint a fresh, single-use Snowflake Streamlit embed URL.

    IMPORTANT:
    The returned URL is never stored in a database, file, or
    environment variable.  Snowflake treats it as a bearer
    credential and its authorization code is short-lived and
    single-use.
    """

    token, role = _get_ask_cortex_embed_credentials()

    parent_origin = _get_ask_cortex_parent_origin(
        request
    )

    account_url = _get_snowflake_account_url_for_embed()

    database = requests.utils.quote(
        ASK_CORTEX_DATABASE,
        safe=""
    )

    schema = requests.utils.quote(
        ASK_CORTEX_SCHEMA,
        safe=""
    )

    streamlit_name = requests.utils.quote(
        ASK_CORTEX_STREAMLIT,
        safe=""
    )
    ...
    endpoint = (
        ...
        f"/schemas/{schema}"
        f"/streamlits/{streamlit_name}"
        f":generate-embed-url"
    )
    ...
    if response.status_code != 200:
        ...
        raise RuntimeError(
            "Snowflake generate-embed-url failed "
            f"({response.status_code}): {detail}"
        )

    try:
        embed_url = response.json().get(
            "embed_url"
        )
    except Exception as parse_error:
        raise RuntimeError(
            "Snowflake returned an invalid embed-url response: "
            f"{parse_error}"
        )

    if not embed_url:
        raise RuntimeError(
            "Snowflake did not return an embed_url."
        )
...
def ask_cortex_share_page(request: Request):
    """
    Public Ask Cortex share page.

    Opening this URL does NOT require the browser user to have
    a Snowflake account.  The backend authenticates to Snowflake,
    mints a fresh one-time embed URL, and places it in an iframe.

    The embed URL itself is deliberately not persisted or cached.
    """

    try:
        embed_url = _mint_ask_cortex_embed_url(
            request
        )
        ...
        print(
            "ASK CORTEX EMBED ERROR: "
            f"{type(embed_error).__name__}: {embed_error}"
        )
...
<body>
    <div class="error">
        <h2>Ask Cortex could not be loaded</h2>
        <p>
            The backend could not create the Snowflake Streamlit
            embed session.
        </p>
        <p>
            Check the backend Snowflake embed configuration and
            Streamlit EMBED privileges.
        </p>
        <code>{html.escape(str(embed_error))}</code>
    </div>
</body>
</html>
""",
...
"""
    ...
    return HTMLResponse(
        ...
        status_code=200,
        headers={
            # The embed URL is single-use, so this page must not
            # be cached by the browser, CDN, or reverse proxy.
            "Cache-Control": (
...
def chat(request: ChatRequest):
    ...
    # ------------------------------------------------------
    # Power BI dataset RLS: the report author can put RLS on any
    # column, not just region - the visual tells us which column
    # (rls_column) and which value(s) the current viewer is
    # authorized to see (rls_values).
    # ------------------------------------------------------

    rls_filter = format_rls_filter(
        request.rls_column,
        request.rls_values
    )
    ...
    if rls_filter:
        ...
        user_query += (
            f" (Authorized RLS filter: {rls_filter})"
        )
            ...
            try:
                ...
                try:
                    ...
                    rca_result = generate_rca_ai_response(
                        ...
                        rls_column=request.rls_column,

                        rls_values=request.rls_values,
